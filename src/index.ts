...
export let reactiveTextArea = (function () {
  ...
  let defaults: defaultTypes = {
    ...
    totalAllowedEl = defaults.helpers?.totalCountEl
      ...
      : null,
    currentEl = defaults.helpers?.curCountEl
      ? document.querySelector(defaults.helpers.curCountEl)
      : null,
    remainderEl = defaults.helpers?.remCountEl
      ? document.querySelector(defaults.helpers.remCountEl)
      : null,
    filterCountEl = defaults.helpers?.filterCountEl
      ? document.querySelector(defaults.helpers.filterCountEl)
      : null,
    updater = (el: Element | null, val: number) => {
      if (el) {
    ...
    caller = {
      customFn: function (callback: object, args: object) {
    ...
    main = {
      ...
      truncateString: function (inputString: string, limit: number) {
      ...
      process: function (
        el: HTMLTextAreaElement,
        events: Partial<KeyboardEvent & Event>
      ) {
        ...
        var newElValue: string = el.value;
        // Counter type helper
        var textCount: number = newElValue.length,
          wordCount: number = newElValue.split(" ").filter(function (word) {
            return word !== "";
          }).length,
          setTypeCount: number = (defaults.countOrLimitType = "text")
            ? textCount
            : wordCount;
